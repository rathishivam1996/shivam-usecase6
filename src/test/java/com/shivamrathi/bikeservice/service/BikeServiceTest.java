...
public class BikeServiceTest {
	...
	private BikeServiceEntity entity;
	...
	public void setUp() {
		...
		entity = BikeServiceEntity.builder().id((long) 1).address("goa").phoneNumber("0123456789").givenDate(date)
				.expectedDeliveryDate(date).createdDate(date).updatedDate(date).bikeChassisNumber("cha1")
	...
	public void should_save_and_return_one_entity() {
		...
		when(repository.save(any(BikeServiceEntity.class))).thenReturn(entity);

		// Act
		BikeServiceEntity actual = service.saveBikeServiceEntity(entity);
		System.out.println("*********************************");
	...
	public void should_find_by_id_and_return_one_entity()
	{
		...
		// Act
		BikeServiceEntity actual = service.findById(getRandomLong());
		System.out.println("*********************************");
	...
	{
		// Setup
		when(repository.findByBikeChassisNumber(anyString())).thenReturn(Optional.of(entity));
		
		// Act
		BikeServiceEntity actual = service.findByBikeChassisNumber("cha1");
		System.out.println("*********************************");
	...
	{
		// Setup
		when(repository.findByBikeRegistrationNumber(anyString())).thenReturn(Optional.of(entity));
		
		// Act
		BikeServiceEntity actual = service.findByBikeRegistrationNumber("cha1");
		System.out.println("*********************************");
	...
	void givenEntityObject_whenUpdateEntity_thenReturnUpdatedEntity() {
		...
		BikeServiceEntity updated = service.updateBikeServiceEntity(entity.getId(), entity);
